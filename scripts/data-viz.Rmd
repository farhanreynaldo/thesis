---
title: "Data visualization"
author: "Farhan Reynaldo"
date: "2024-09-26"
output: html_document
---

## Load library and data

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
here::i_am("scripts/data-viz.Rmd")

library(ggtext)
library(usmap)
library(sf)
library(tidyverse)
library(here)

source(here::here("src/theme.R"))
source(here::here("src/load_data.R"))
source(here::here("src/election.R"))

map_data <- usmap::us_map(regions = "states")
poststrat_data <- read_csv(here("data/census-pums-pop.csv"))
```

```{r}
COVARIATES <- c("stt", "eth", "inc", "age", "educ", "metro")

get_label <- function(col_name) {
  label <- switch(col_name,
    eth = eth.label,
    inc = inc.label,
    age = age.label,
    educ = educ.label,
    metro = metro.label,
    stt = stt.label
  )
  return(label)
}

relevel_label <- function(col_name) {
  col_name <- deparse(substitute(col_name))
  get_label(col_name)
}

map_label <- function(col) {
  col_name <- deparse(substitute(col))
  label <- get_label(col_name)
  label_mapper <- setNames(label, 1:length(label))
  result <- unname(label_mapper[as.character(col)])
  fct_relevel(result, label)
}

create_adjusted_data_matrix <- function(model_matrix, poststrat_data = NULL) {
  year <- model_matrix$data_matrix$year[1]
  year_pop_column <- paste0("wtpop", year)

  data_matrix <- model_matrix$data_matrix %>%
    mutate(pred = colMeans(model_matrix$pred))
  
  poststrat_data %>%
    mutate(grp = apply(.[, COVARIATES], 1, paste, collapse = "_"),
           pop = year_pop_column) %>% 
    dplyr::select(grp, pop = !!year_pop_column) %>% 
    right_join(data_matrix, by = "grp") %>% 
    group_by(stt) %>% 
    mutate(
      vote = weighted_correction(
        pred,
        pop / sum(pop),
        first(rep_votes)
      )$corrected
    ) %>% 
    ungroup()
}

poststrat_by <- function(..., data_matrix, year) {
  vars <- c(...)
  
  data_matrix %>% 
    group_by(across(all_of(c("year", vars)))) %>%
    summarise(
      vote_all = sum(vote * pop, na.rm = TRUE) / sum(pop, na.rm = TRUE),
      vote_white = sum(vote[eth == 1] * pop[eth == 1], na.rm = TRUE) / sum(pop[eth == 1], na.rm = TRUE),
      se_all = sqrt((vote_all * (1 - vote_all)) / sum(n)),
      se_white = sqrt((vote_white * (1 - vote_white)) / sum(n[eth == 1])), 
      .groups = "drop"
    ) %>%
        mutate(across(is.factor, map_label, .names = "{.col}_str"))
}
```

```{r}
model_matrix_2012 <- readRDS("../data/model/second-iteration/model_matrix_complete-2012-formula_level_4.rds")
model_matrix_2012$pred <- readRDS("../data/model/second-iteration/pred_complete-2012-formula_level_4.rds")

model_matrix_2016 <- readRDS("../data/model/second-iteration/model_matrix_complete-2016-formula_level_4.rds")
model_matrix_2016$pred <- readRDS("../data/model/second-iteration/pred_complete-2016-formula_level_4.rds")

model_matrix_2020 <- readRDS("../data/model/second-iteration/model_matrix_complete-2020-formula_level_4.rds")
model_matrix_2020$pred <- readRDS("../data/model/second-iteration/pred_complete-2020-formula_level_4.rds")

model_matrices <- list(model_matrix_2012, model_matrix_2016, model_matrix_2020)
rm(model_matrix_2012, model_matrix_2016, model_matrix_2020)

years <- c(2012, 2016, 2020)
data_matrices <- map(model_matrices, create_adjusted_data_matrix, poststrat_data = poststrat_data)

# Solely for sorting the state facet map
states_data <- process_states_data(states_election_data, states_pop_income_data, 2012)
```

## Income-state

```{r, fig.width = 10, fig.height = 10}
income_state <- map2(data_matrices, years, ~ poststrat_by("inc", "stt", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  left_join(stt_dict, by = c("stt" = "stt")) %>%
  filter(!state_abbr %in% c("AK", "HI", "DC")) %>%
  left_join(states_data %>% dplyr::select(state_abbr, rep_votes), by = c("state_abbr" = "state_abbr")) %>% 
  mutate(state_name = fct_reorder(state_name, rep_votes, .desc = TRUE)) %>% 
  mutate(year = as.factor(year)) 

income_state %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = inc, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~state_name, ncol = 8) + 
  scale_x_discrete(labels = c("poor", "", "mid", "", "rich")) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 12),
    legend.position = "top",
    legend.text = element_text(size = 14),
    legend.title = element_text(size = 14)
  )


ggsave(here("figures/model/income-state.svg"), width = 12, height = 10)
ggsave(here("figures/model/income-state.png"), width = 12, height = 10, dpi = 300)
```
## Educ-state

```{r, fig.width = 10, fig.height = 10}
educ_state <- map2(data_matrices, years, ~poststrat_by("educ", "stt", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  left_join(stt_dict, by = c("stt" = "stt")) %>%
  filter(!state_abbr %in% c("AK", "HI", "DC")) %>%
  left_join(states_data %>% dplyr::select(state_abbr, rep_votes), by = c("state_abbr" = "state_abbr")) %>% 
  mutate(state_name = fct_reorder(state_name, rep_votes, .desc = TRUE)) %>%
  mutate(year = as.factor(year)) 

educ_state %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = educ, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~state_name, ncol = 7) + 
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_x_discrete(labels = c("<HS", "", "", "", "Postgrad")) +
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top",
    legend.text = element_text(size = 14),
    legend.title = element_text(size = 14),
    axis.text.x = element_text(size = 7)
  )

ggsave(here("figures/model/educ-state.svg"), width = 12, height = 10)
ggsave(here("figures/model/educ-state.png"), width = 12, height = 10, dpi = 300)
```

## Educ-inc

```{r}
educ_inc <- map2(data_matrices, years, ~poststrat_by("educ", "inc", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

educ_inc %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = educ_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~inc_str) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )

ggsave(here("figures/model/educ-state.svg"), width = 12, height = 10)
ggsave(here("figures/model/educ-state.png"), width = 12, height = 10, dpi = 300)
```

## Educ-inc-eth

```{r, width = 12, height = 8}
educ_inc <- map2(data_matrices, years, ~poststrat_by("educ", "inc", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

educ_inc %>%
  ggplot(aes(group = year)) +
  geom_line(aes(x = inc_str, y = vote_all), linewidth = 1, alpha = 0.8, color = "grey") + 
  geom_line(aes(x = inc_str, y = pred_white), linewidth = 1, alpha = 0.8, color = "orange") +
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_grid(vars(year), vars(educ_str)) + 
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 7),
    axis.text.x = element_text(size = 5),
    legend.position = "top"
  )

ggsave(here("figures/model/inc-educ-eth.svg"), width = 8, height = 6)
ggsave(here("figures/model/inc-educ-eth.png"), width = 8, height = 6, dpi = 300)
```

## metro-inc-eth

```{r}
metro_inc <- map2(data_matrices, years, ~poststrat_by("metro", "inc", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_inc %>%
  ggplot(aes(group = year)) +
  geom_line(aes(x = inc_str, y = vote_all), linewidth = 1, alpha = 0.8, color = "grey") + 
  geom_line(aes(x = inc_str, y = pred_white), linewidth = 1, alpha = 0.8, color = "orange") +
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_grid(vars(year), vars(metro_str)) + 
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 7),
    axis.text.x = element_text(size = 5),
    legend.position = "top"
  )

ggsave(here("figures/model/inc-metro-eth.svg"), width = 8, height = 6)
ggsave(here("figures/model/inc-metro-eth.png"), width = 8, height = 6, dpi = 300)
```

## metro-educ-eth

```{r}
metro_educ <- map2(data_matrices, years, ~poststrat_by("metro", "educ", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_educ %>%
  ggplot(aes(group = year)) +
  geom_line(aes(x = educ_str, y = vote_all), linewidth = 1, alpha = 0.8, color = "grey") + 
  geom_line(aes(x = educ_str, y = pred_white), linewidth = 1, alpha = 0.8, color = "orange") +
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_grid(vars(year), vars(metro_str)) + 
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 7),
    axis.text.x = element_text(size = 5),
    legend.position = "top"
  )

ggsave(here("figures/model/educ-metro-eth.svg"), width = 8, height = 6)
ggsave(here("figures/model/educ-metro-eth.png"), width = 8, height = 6, dpi = 300)
```

```{r}
educ_inc <- map2(data_matrices, years, ~poststrat_by("educ", "inc", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

educ_inc %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = inc_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~educ_str) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )

ggsave(here("figures/model/income-educ.svg"), width = 12, height = 10)
ggsave(here("figures/model/income-educ.png"), width = 12, height = 10, dpi = 300)
```

## Metro-inc

```{r}
metro_inc <- map2(data_matrices, years, ~poststrat_by("metro", "inc", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_inc %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = metro_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~inc_str) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )

ggsave(here("figures/model/metro-income.svg"), width = 12, height = 10)
ggsave(here("figures/model/metro-income.png"), width = 12, height = 10, dpi = 300)
```


```{r, fig.width = 8, fig.height = 6}
metro_inc <- map2(data_matrices, years, ~poststrat_by("metro", "inc", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_inc %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = inc_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~metro_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )

ggsave(here("figures/model/income-metro.svg"), width = 8, height = 6)
ggsave(here("figures/model/income-metro.png"), width = 8, height = 6, dpi = 300)
```


## Educ-metro

```{r}
educ_metro <- map2(data_matrices, years, ~poststrat_by("educ", "metro", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

educ_metro %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = educ_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~metro_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )

ggsave(here("figures/model/educ-metro.svg"), width = 12, height = 10)
ggsave(here("figures/model/educ-metro.png"), width = 12, height = 10, dpi = 300)
```

## Income-eth

```{r}
inc_eth <- map2(data_matrices, years, ~poststrat_by("inc", "eth", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

inc_eth %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = inc_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~eth_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```

## Educ-age

```{r}
educ_age <- map2(data_matrices, years, ~poststrat_by("educ", "age", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

educ_age %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = educ_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~age_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```
## Educ-eth

```{r}
educ_eth <- map2(data_matrices, years, ~poststrat_by("educ", "eth", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

educ_eth %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = educ_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~eth_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```

## Metro-state

What should we do in poststratification step if one of the subgroups has 0 population? In this case, I want to plot how urban and rural differs in each state across years. However, states, such as Rhode Island and New Jersey does not have rural population (based on the IPUMS data). Is it acceptable to use raw prediction from the model without poststratification? 

```{r, fig.width = 10, fig.height = 10}
metro_state <- map2(data_matrices, years, ~poststrat_by("metro", "stt", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  left_join(stt_dict, by = c("stt" = "stt")) %>%
  filter(!state_abbr %in% c("AK", "HI", "DC")) %>%
  left_join(states_data %>% dplyr::select(state_abbr, rep_votes), by = c("state_abbr" = "state_abbr")) %>% 
  mutate(state_name = fct_reorder(state_name, rep_votes, .desc = TRUE)) %>%
  mutate(year = as.factor(year)) 

metro_state %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = metro, y = vote_all), linewidth = 0.7, alpha = 0.8) + 
  geom_point(aes(x = metro, y = vote_all), size = 0.5, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~state_name, ncol = 7) + 
  scale_x_discrete(labels = c("Rural", "Suburban", "Urban")) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each rural-urban category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top",
    legend.text = element_text(size = 14),
    legend.title = element_text(size = 14),
    axis.text.x = element_text(size = 6)
  )

ggsave(here("figures/metro-state.svg"), width = 12, height = 10)
ggsave(here("figures/metro-state.png"), width = 12, height = 10, dpi = 300)
```

## Metro-educ

```{r}
metro_educ <- map2(data_matrices, years, ~poststrat_by("metro", "educ", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_educ %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = metro_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~educ_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```
## Metro-eth

```{r}
metro_eth <- map2(data_matrices, years, ~poststrat_by("metro", "eth", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_eth %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = metro_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~eth_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```
## Metro-age

```{r}
metro_age <- map2(data_matrices, years, ~poststrat_by("metro", "age", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

metro_age %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = metro_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~age_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```

## Income-age

```{r}
inc_age <- map2(data_matrices, years, ~poststrat_by("inc", "age", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  mutate(year = as.factor(year)) 

inc_age %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = inc_str, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~age_str, ncol = 2) +
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top"
  )
```

## Age-state

```{r, fig.width = 10, fig.height = 10}
age_state <- map2(data_matrices, years, ~poststrat_by("age", "stt", data_matrix = .x, year = .y)) %>%
  list_rbind() %>% 
  left_join(stt_dict, by = c("stt" = "stt")) %>%
  filter(!state_abbr %in% c("AK", "HI", "DC")) %>%
  left_join(states_data %>% dplyr::select(state_abbr, rep_votes), by = c("state_abbr" = "state_abbr")) %>% 
  mutate(state_name = fct_reorder(state_name, rep_votes, .desc = TRUE)) %>%
  mutate(year = as.factor(year))

age_state %>%
  ggplot(aes(group = year, color = year)) + 
  geom_line(aes(x = age, y = vote_all), linewidth = 1, alpha = 0.8) + 
  geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
  facet_wrap(~state_name, ncol = 7) + 
  scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
  scale_x_discrete(labels = age.label) +
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
  coord_cartesian(ylim = c(0, 1)) +
  labs(
    x = "", y = "", 
    # title = "Republican vote share for each education category across states-year"
  ) + 
  theme_classic() +
  theme(
    plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
    plot.caption = element_text(hjust = 0.5, margin = margin(10, 0, 0, 0)),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    strip.text = element_text(size = 10),
    legend.position = "top",
    legend.text = element_text(size = 14),
    legend.title = element_text(size = 14),
    axis.text.x = element_text(size = 7)
  )

ggsave(here("figures/age-state.svg"), width = 12, height = 10)
ggsave(here("figures/age-state.png"), width = 12, height = 10, dpi = 300)
```

## Generate all plots

```{r}
vars <- c("stt", "eth", "inc", "age", "educ", "metro")
combinations <- expand.grid(axis = vars, facet = vars) %>% 
  filter(axis != facet, axis != "stt") %>% 
  mutate(across(everything(), as.character))

# Loop through each combination and generate data frame and plot
for (i in 1:nrow(combinations)) {
  var1 <- combinations$axis[i]
  var2 <- combinations$facet[i]
  
  # Generate a unique name for the current combination
  combination_name <- paste0(var1, "_", var2)
  
  # Compute the post-stratified data for each year
  post_data <- map2(data_matrices, years, ~poststrat_by(var1, var2, data_matrix = .x, year = .y))
  
  # Combine results into a single data frame
  if (var2 == "stt") {
    combined_data <- post_data %>% 
      list_rbind() %>% 
      left_join(stt_dict, by = c("stt" = "stt")) %>%
      filter(!state_abbr %in% c("AK", "HI", "DC")) %>%
      left_join(states_data %>% dplyr::select(state_abbr, rep_votes), by = c("state_abbr" = "state_abbr")) %>% 
      mutate(state_name = fct_reorder(state_name, rep_votes, .desc = TRUE)) %>%
      mutate(year = as.factor(year))
  } else {
    combined_data <- post_data %>% 
      list_rbind() %>% 
      mutate(year = as.factor(year))
  }
  
  var1_str <- paste0(var1, "_str")
  var2_str <- as.formula(paste0("~", var2, "_str"))
  
  # Plotting the result
  plot <- combined_data %>% 
    ggplot(aes(group = year, color = year)) + 
    geom_line(aes(x = .data[[var1_str]], y = vote_all), linewidth = 1, alpha = 0.8) + 
    geom_hline(aes(yintercept = 0.5), color = "grey80", linewidth = 0.2) +
    # facet_wrap(var2_str, ncol = 7) + 
    scale_color_manual(values = c("#FCBE3A", "#FCA53A", "#FF5C33")) + 
    scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 1, 0.5)) + 
    coord_cartesian(ylim = c(0, 1)) +
    labs(
      x = "", y = "", 
      title = paste("Republican vote share by", var1, "and", var2, "across states and years")
    ) + 
    theme_classic() +
    theme(
      plot.title = element_text(size = 16, hjust = 0.5, margin = margin(0, 0, 20, 0)),
      panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(),
      strip.text = element_text(size = 10),
      legend.position = "top",
      legend.text = element_text(size = 14),
      legend.title = element_text(size = 14),
      axis.text.x = element_text(size = 7)
    )
  
  if (var2 == "stt") {
    plot <- plot +
      facet_wrap(var2_str, ncol = 7)
    ggsave(paste0("../figures/model/", "plot_", combination_name, ".png"), plot = plot, width = 10, height = 10)
  } else {
    plot <- plot +
      facet_wrap(var2_str)
    ggsave(paste0("../figures/model/", "plot_", combination_name, ".png"), plot = plot)
  }
}
```

```{r, fig.width = 10, fig.height = 7}
# Assuming each model matrix in model_matrices contains a 'model' object for the MCMC
plots <- map2(data_matrices, years, ~ {
  rhat_values <- bayesplot::rhat(.x$model)
  plot <- bayesplot::mcmc_rhat_hist(rhat_values) + 
    ggtitle(paste("Rhat diagnostic for election model", .y))
  return(plot)
})

# patchwork::wrap_plots(plots, ncol = 2, guides = "collect")
# ggsave(here("figures/rhat.png"), width = 10, height = 7, dpi = 300)
```